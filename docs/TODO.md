...
- [x] 이모지 피커 라이브러리 설치
  - [x] `emoji-picker-react` 또는 다른 라이브러리 설치
  - [x] 필요한 타입 정의 확인
- [x] CommentForm 컴포넌트에 이모지 피커 기능 추가
  - [x] 이모지 버튼 활성화 (현재 비활성화 상태)
  - [x] 이모지 피커 표시/숨김 상태 관리
  - [x] 이모지 선택 시 textarea에 삽입 기능
  - [x] 외부 클릭 시 피커 닫기 기능 (useRef, useEffect 활용)
  - [x] 이모지 피커 위치 조정 (버튼 위쪽에 표시)
- [x] UI/UX 개선
  - [x] 이모지 피커 스타일링 (Instagram 디자인에 맞게)
  - [x] 모바일 반응형 대응
  - [x] 로그 추가 (디버깅용)
...
- [x] 프로필 헤더 컴포넌트 (`components/profile/ProfileHeader.tsx`)
  ...
  - [x] "프로필 편집" 버튼 (내 프로필일 때)
    - [x] EditProfileModal 컴포넌트 생성
    - [x] 프로필 편집 API (PATCH 메서드)
    - [x] 프로필 헤더에 편집 버튼 추가
...
- [x] 검색 기능
  ...
  - [x] (선택) 게시물 검색 기능
    - [x] 캡션으로 게시물 검색
    - [x] 검색 결과에 게시물 미리보기 표시
    - [x] 클릭 시 게시물 상세 페이지로 이동
    - [x] 검색 타입 선택 탭 (사용자/게시물)
    - [x] PostSearchResult 컴포넌트 생성
