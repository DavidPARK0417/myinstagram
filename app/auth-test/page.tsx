...
export default function AuthTestPage() {
  ...
  const testConnection = useCallback(async () => {
    try {
      ...
      // Supabase 에러 객체는 표준 Error가 아니므로 상세 정보를 로깅
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : (err as any)?.message || (err as any)?.code || "연결 테스트 실패";
  ...
  const fetchOrCreateUser = useCallback(async () => {
    ...
    try {
      ...
      // Supabase 에러 객체는 표준 Error가 아니므로 상세 정보를 로깅
      console.group("🔴 Fetch or create user error");
      console.error("Error object:", err);
      
      // Supabase 에러 객체의 속성 확인
      if (err && typeof err === "object") {
        console.error("Error code:", (err as any).code);
        console.error("Error message:", (err as any).message);
        console.error("Error details:", (err as any).details);
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : (err as any)?.message || (err as any)?.code || "사용자 데이터 조회/생성 실패";
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  const updateName = async () => {
    ...
    try {
      ...
      // Supabase 에러 객체는 표준 Error가 아니므로 상세 정보를 로깅
      console.group("🔴 Update name error");
      console.error("Error object:", err);
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : (err as any)?.message || (err as any)?.code || "이름 업데이트 실패";
      setError(errorMessage);
    }
  };
