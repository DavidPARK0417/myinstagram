 ...
 *
 * 이 API는 특정 사용자가 팔로우하고 있는 사용자들의 목록을 조회합니다.
 *
 * GET: 팔로잉 목록 조회
...
export async function GET(
  ...
  try {
    console.group(
      "[API] GET /api/users/[userId]/following - 팔로잉 목록 조회 시작",
    );
    ...
    if (followsError) {
      console.error("❌ 팔로잉 목록 조회 실패:", followsError);
      console.groupEnd();
      return NextResponse.json(
        {
          error: "Database Error",
          message: "팔로잉 목록을 조회하는 중 오류가 발생했습니다.",
    ...
    console.log("📝 팔로잉 ID별 사용자 조회 시작:", {
      followingIds,
      count: followingIds.length,
    });

    // 각 팔로잉 ID에 대해 개별적으로 사용자 정보 조회
    for (const followingId of followingIds) {
      try {
        console.log(`🔍 팔로잉 ID ${followingId}에 대한 사용자 조회`);
        ...
        if (userError) {
          console.error(`❌ 사용자 ${followingId} 조회 실패:`, userError);
        ...
        console.error(`❌ 사용자 ${followingId} 조회 중 예외:`, error);
        // 개별 사용자 조회 실패는 전체 실패로 처리하지 않음
        continue;
      }
    }
    ...
    console.log("✅ 팔로잉 목록 조회 성공:", {
      ...
      requestedIds: followingIds.length,
    });
    console.groupEnd();
